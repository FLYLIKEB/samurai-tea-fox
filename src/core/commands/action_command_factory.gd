extends RefCounted
class_name ActionCommandFactory

const GameCommand = preload("res://src/core/commands/game_command.gd")

static func movement_command(direction: Vector2i):
	return GameCommand.new(GameCommand.Type.MOVE, direction)

static func command_for_action(action: String, direction := Vector2i.ZERO, slot := 0, payload := {}):
	match action:
		"attack":
