...
func start_drinking(slot_index: int, context := {}) -> Dictionary:
	...
	var action := {
		...
		"vessel_instance_id": String(prepared.get("vessel_instance_id", "")),
		"elapsed_seconds": 0.0,
...
func _build_prepared_tea(tea_id: String, modifier_query: Dictionary, context: Dictionary) -> Dictionary:
	...
	return {"ok": true, "prepared_tea": {
		...
		"vessel_instance_id": String(vessel.get("vessel_instance_id", "")),
		"tea_name": tea.name,
		"vessel_name": vessel.vessel_name,
		"attachment_description_key": String(vessel.get("attachment_description_key", "")),
...
func _validate_modifier_query(query: Dictionary) -> Dictionary:
	...
	return {"ok": true, "modifier": {
		...
		"vessel_instance_id": String(query.get("vessel_instance_id", "")),
		"attachment_description_key": String(query.get("attachment_description_key", "")),
