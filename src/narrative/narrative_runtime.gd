...
func from_catalog(catalog) -> Dictionary:
	...
	for row in catalog.get_definitions("events"):
		var result := _event_definition_from_row(row)
		if not result.ok:
			return result
...
func select_option(event_id: String, node_id: String, option_id: String, run_state, meta_state = null) -> Dictionary:
	var node_result := _node_for_event(event_id, node_id)
	if not node_result.ok:
		return node_result
	var run_query := _run_query(run_state)
...
func _event_definition_from_row(row: Dictionary) -> Dictionary:
	...
	var nodes_result := _nodes_from_row(row)
...
func _nodes_from_row(row: Dictionary) -> Dictionary:
	...
	for raw_node in raw_nodes:
		...
		var options_result := _options_from_node(row, raw_node)
...
func _options_from_node(row: Dictionary, raw_node: Dictionary) -> Dictionary:
