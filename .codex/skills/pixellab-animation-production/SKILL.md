...
description: 무차우 캐릭터·오브젝트의 움직임, 방향 전환, 애니메이션 스프라이트를 PixelLab API로 만들고 검증할 때 사용한다.
metadata:
...
무차우에서 캐릭터, 몬스터, 오브젝트의 걷기·대기·공격·상호작용 같은 움직임이 필요한 스프라이트를 만들 때 사용한다. 일반 단일 이미지 생성보다 PixelLab의 캐릭터·애니메이션 전용 API를 우선한다.
...
- 무차우 주인공은 큰 붉은여우 귀와 꼬리, 작은 검, 차통 또는 다구, 절제된 사무라이 실루엣을 유지한다.
...
- 맵·사물과 정면 전용 캐릭터는 측면·후면·3/4 방향을 만들지 않는다. 캐릭터 이동용 방향 세트가 명시된 작업은 기존 정면을 south 기준 이미지로 고정하고 west/east/north만 추가하며, 3/4·아이소메트릭 방향은 만들지 않는다.
