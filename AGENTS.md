...
이 저장소는 **Godot 4.x + GDScript** 기반 2D 픽셀 로그라이크 게임 **Samurai Tea Fox / 무차우: 한 잔의 도** 프로젝트다.
...
- 캐릭터, 맵, 맵 내 사물의 기본 표현은 정사각형 타일 기반 탑뷰 로그라이크 가독성을 위해 정면을 우선한다. 맵·사물은 측면, 후면, 3/4, 아이소메트릭 시점으로 만들지 않으며, 캐릭터는 이동용 방향 세트가 명시된 경우에만 같은 정면 원본을 기준으로 좌·우·후면을 추가할 수 있다.
- AI로 생성한 원본 raw 이미지와 중간 후보는 모두 `assets/source/imagegen/` 아래에 둔다. 이 폴더는 Git에서 제외하며, 검수 완료된 최종 런타임 PNG만 `assets/sprites/`, `assets/tiles/`, `assets/ui/`로 승격한다.
