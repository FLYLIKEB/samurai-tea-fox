...
func run(asserts) -> void:
	var run_save := SaveCodec.encode_run({"seed": 11037, "inventory": ["wood"], "equipment": {"slots": {}}})
	...
	asserts.equal(SaveCodec.decode_run(run_save).state.equipment, {"slots": {}}, "run save preserves equipment payload")
