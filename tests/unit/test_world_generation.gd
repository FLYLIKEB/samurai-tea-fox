...
func run(asserts) -> void:
	...
	asserts.equal(_resource_count(a.resource_nodes, "bandage"), 1, "common map places one seed-positioned bandage pickup")
	asserts.true_value(_landmarks_of_kind(a.world_data, WorldData.LANDMARK_ABANDONED_HOUSE).size() in [3, 4], "each map has three or four searchable abandoned houses")
	asserts.equal(a.connectivity.required_landmark_ids.size(), 5 + _landmarks_of_kind(a.world_data, WorldData.LANDMARK_ABANDONED_HOUSE).size(), "entry, travel ruin, abandoned houses, teleport, core dungeon, and boss anchor are required")
