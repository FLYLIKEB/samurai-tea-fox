...
func _assert_mobile_controls_emit_shared_commands(asserts) -> void:
	...
	asserts.true_value(hud.press_mobile_button("repair_teleport"), "HUD accepts teleport repair command control")
	asserts.equal(received.size(), 16, "HUD emits exactly one command per valid mobile control")
	...
	asserts.equal(received[15].type, GameCommand.Type.REPAIR_TELEPORT, "teleport repair control emits shared repair command")
