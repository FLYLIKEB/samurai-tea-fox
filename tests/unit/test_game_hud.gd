...
func _assert_mobile_controls_emit_shared_commands(asserts) -> void:
	...
	asserts.equal(received.size(), 11, "HUD emits exactly one command per valid mobile control")
	...
	asserts.equal(received[10].type, GameCommand.Type.OPEN_MAP, "map control emits shared map menu command")
