...
func _assert_equipment_strip_reads_runtime_equipment(asserts) -> void:
	...
	asserts.equal(snapshot.weapon.display_text, "무", "weapon slot keeps a compact visible slot cue")
	asserts.equal(snapshot.armor.display_text, "방", "armor slot keeps a compact visible slot cue")
	asserts.equal(snapshot.tea_ware.display_text, "다", "tea ware slot keeps a compact visible slot cue")
	...
	asserts.equal(snapshot.weapon.display_text, "무", "empty equipment slot keeps its visible slot cue")
...
func _assert_command_button_factories_preserve_contracts(asserts) -> void:
	...
	placement.pressed.emit()
	asserts.equal(received.size(), 2, "placement command button emits exactly one command per press")
