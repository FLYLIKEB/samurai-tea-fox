...
func _assert_mobile_controls_emit_shared_commands(asserts) -> void:
	...
	asserts.equal(received.size(), 12, "HUD emits exactly one command per valid mobile control")
	...
	asserts.equal(received[9].type, GameCommand.Type.OPEN_CRAFTING, "crafting control emits shared crafting menu command")
	asserts.equal(received[10].type, GameCommand.Type.OPEN_FACILITIES, "facilities control emits shared facilities menu command")
	asserts.equal(received[11].type, GameCommand.Type.OPEN_MAP, "map control emits shared map menu command")
