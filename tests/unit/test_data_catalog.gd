...
func run(asserts) -> void:
	...
	asserts.equal(catalog.data_version, "notion-2026-09-04", "data version is pinned")
	...
	asserts.equal(catalog.find_by_id("dungeons", "dungeon_4").get("boss_id", ""), "chr_3", "canonical dungeon row keeps boss character stable id")
	asserts.equal(catalog.find_by_id("dungeons", "dungeon_4").get("pre_boss_dialogue_event_id", ""), "story_b01_03", "canonical dungeon row keeps explicit pre-boss dialogue event")
