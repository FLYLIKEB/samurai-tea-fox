...
func run(asserts) -> void:
	...
	asserts.equal(home.get("fence_owner_ids", []).size(), 5, "fox home keeps a partial decorative fence with an open front")
	...
	for owner_id in WorldGenerator.STARTING_HOME_BRIDGE_IDS:
		...
		asserts.equal(reservation.get("size", {}), {"x": 2, "y": 1}, "%s stays a short, narrow bridge" % owner_id)
	...
	asserts.false_value(world.is_walkable(entry_position + Vector2i(1, -1)), "narrow eastern creek stays impassable away from the bridge")
	asserts.true_value(world.terrain_id_at(entry_position + Vector2i(0, -1)) != WorldGenerator.TERRAIN_WATER, "creek bank does not use the water terrain")
	asserts.true_value(world.is_walkable(entry_position + Vector2i(-4, 0)), "starting clearing is open to the west instead of fully moated")
	asserts.true_value(ConnectivityValidator.new().validate_world_data(generated.world_data).valid, "starting clearing and bridge preserve required landmark connectivity")
