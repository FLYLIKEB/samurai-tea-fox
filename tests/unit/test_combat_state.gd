...
func _assert_generated_catalog_loads_combat_values(asserts) -> void:
	...
	asserts.equal(config.ki_attack_multiplier_0, 0.9, "low ki multiplier comes from balance data")
	asserts.equal(config.ki_attack_multiplier_100, 1.5, "high ki multiplier comes from balance data")
...
func _assert_combo_damage_events_and_duplicate_prevention(asserts) -> void:
	...
	asserts.true_value(second.ok, "repeated attack input starts immediately without delay")
	combat.tick(0.8)
