...
func _assert_crafting_read_model_reports_filters_detail_and_reasons(asserts) -> void:
	...
	var all_model: Dictionary = service.read_model(inventory, {"unlocked_biome_ids": ["common_region"]})
	asserts.true_value(bool(all_model.rows[0].craftable) and bool(all_model.rows[1].craftable), "all recipes list places craftable recipes first")
	asserts.equal(all_model.rows[0].recipe_id, "crowded_bowl", "craftable recipes keep stable id ordering")
	asserts.equal(all_model.rows[1].recipe_id, "wooden_workbench", "all craftable recipes appear before unavailable recipes")
	asserts.false_value(bool(all_model.rows[2].craftable), "unavailable recipes follow the craftable group")
