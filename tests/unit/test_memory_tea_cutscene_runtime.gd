...
func _assert_memory_tea_completion_starts_cutscene(asserts, catalog: DataCatalog) -> void:
	...
	asserts.equal(run_state.discovered_records, ["memory_tea", "memory_tea_father_spring_pan_fired_tea"], "run discovery records aggregate and specific memory once")
...
func _assert_skip_records_once_and_blocks_replay(asserts) -> void:
	...
	asserts.equal(run_state.discovered_records, ["memory_tea", "memory_tea_fixture"], "skip records aggregate and specific discovery")
	...
	asserts.equal(run_state.discovered_records, ["memory_tea", "memory_tea_fixture"], "duplicate discovery remains unique")
...
func _assert_snapshot_resume_then_complete(asserts) -> void:
	...
	asserts.equal(run_state.discovered_records, ["memory_tea", "memory_tea_resume"], "restored completion records discovery")
	...
	asserts.equal(decoded.run_state.discovered_records, ["memory_tea", "memory_tea_resume"], "run discovered records survive save round-trip")
...
func _assert_main_routes_memory_tea_completion(asserts, catalog: DataCatalog) -> void:
	...
	asserts.equal(restored.run_state.discovered_records, ["memory_tea", "memory_tea_father_spring_pan_fired_tea"], "main records aggregate and specific memory discovery once after skip")
	runtime.free()
	restored.free()
